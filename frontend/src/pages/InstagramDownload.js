...
import { FaInstagram, FaDownload, FaEye, FaExclamationTriangle, FaCheckCircle, FaCopy, FaTimes, FaLink, FaSearch, FaSlidersH } from 'react-icons/fa';
import './pages.css';
...
function InstagramDownload() {
  ...
  // Recent downloads for Instagram
  const recentDownloads = (JSON.parse(localStorage.getItem('downloadHistory')) || []).filter(d => d.platform === 'instagram').slice(0, 3);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `instagram_${info.title?.slice(0, 40).replace(/\s+/g, '_') || 'video'}.mp4`;
      link.click();
    ...
    <div className="page-wrapper">
      <div className="page-center">
        <div className="card fade-in">
          <div className="card-accent" style={{ borderColor: '#C13584' }}>
            <FaInstagram style={{ color: '#C13584' }} />
          </div>
          <h2>Download from Instagram</h2>
          <div className="subtitle">Fast, high-quality Instagram video downloads. No watermark. No hassle.</div>
          <div className="input-row">
            <input type="text" placeholder="Paste Instagram link here" value={url} onChange={(e) => setUrl(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handlePreview()} />
          ...
          <div className="howto">
            ...
            <div className="howto-step"><span className="howto-icon"><FaLink /></span> Paste the Instagram video link above.</div>
          ...
          {recentDownloads.length > 0 && (
            <div className="recent-downloads">
              <div className="recent-downloads-title">Recent Instagram Downloads</div>
