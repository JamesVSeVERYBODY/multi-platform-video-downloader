...
import { FaYoutube, FaDownload, FaEye, FaExclamationTriangle, FaCheckCircle, FaCopy, FaTimes, FaLink, FaSearch, FaSlidersH } from 'react-icons/fa';
import './pages.css';
...
function YouTubeDownload() {
  ...
  // Recent downloads for YouTube
  const recentDownloads = (JSON.parse(localStorage.getItem('downloadHistory')) || []).filter(d => d.platform === 'youtube').slice(0, 3);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `youtube_${info.title?.slice(0, 40).replace(/\s+/g, '_') || 'video'}.mp4`;
      link.click();
    ...
    <div className="page-wrapper">
      <div className="page-center">
        <div className="card fade-in">
          <div className="card-accent" style={{ borderColor: '#FF0000' }}>
            <FaYoutube style={{ color: '#FF0000' }} />
          </div>
          <h2>Download from YouTube</h2>
          <div className="subtitle">Download YouTube videos in any quality. Fast, easy, and free!</div>
          <div className="input-row">
            <input type="text" placeholder="Paste YouTube link here" value={url} onChange={(e) => setUrl(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handlePreview()} />
          ...
          <div className="howto">
            ...
            <div className="howto-step"><span className="howto-icon"><FaLink /></span> Paste the YouTube video link above.</div>
          ...
          {recentDownloads.length > 0 && (
            <div className="recent-downloads">
              <div className="recent-downloads-title">Recent YouTube Downloads</div>
